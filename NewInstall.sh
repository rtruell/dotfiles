...
# installed.  if installing on 'nas' or 'nasbackup', the files and programs are
# directly available and nothing needs to be mounted.  however, even on those
# machines, a variable has to be set with a directory path
...
case "${SYSTEM_TYPE,,}" in
  macos)
         # this will prompt for the user/password, then try to create mountpoint
         # '/Volumes/data' and mount the NAS' data directory there
         ...
         case "${computername}" in
           nas*)
              ...
              *)
                 ...
                 # mount it.  don't forget to change the user name as necessary.
                 # note that this will ask for the password for that user on NAS
...
# if not installing on macOS, copy the samba config file to $HOME...it'll be put
# where it belongs after 'samba' is installed
if [[ "${SYSTEM_TYPE,,}" != "macos" ]]; then
  case "${computername}" in
...
# macOS and Linux systems, with the only variances being the commands used, the
# locations of the files/programs, and that some Linux machines don't get
# things that others do.  but from this point on, nothing that gets installed
# on a macOS system gets installed on a Linux system, and vice versa.  so, now
# OS-dependent scripts get sourced to finish things off
case "${SYSTEM_TYPE,,}" in
